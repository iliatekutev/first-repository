...
app = Flask(__name__)
app.config['SECRET_KEY'] = '5ec6@1LdF1'

login_manager = LoginManager(app)
login_manager.login_view = 'login'

connection = sqlite3.connect("sqlite (3).db", check_same_thread=False)
...
@app.route("/")
def index():
    cursor.execute(''' SELECT post.id, post.title, post.content, post.author_id, post.topic,
    user.username, COUNT(like.id) AS likes FROM post
    JOIN user ON post.author_id = user.id
    LEFT JOIN like ON post.id = like.post_id
    GROUP BY post.id, post.title, post.content, post.topic,
    ...
    for post in reversed(result):
        posts.append({
            ...
            'author_id': post[3], 'topic': post[4], 'username': post[5],
            'likes': post[6]
        })
...
def index2():
    if request.method == 'POST':
        ...
        cursor.execute(
        'INSERT INTO post (title, content, author_id, topic) VALUES (?, ?, ?, ?)',
        (title, content, current_user.id, topic))
        connection.commit()
...
def post(post_id):
    ...
    print(result)
    post_dict = {'id': result[0], 'title': result[1], 'content': result[2], 'topic': result[5]}
...
def like_post(post_id):
    post = cursor.execute('SELECT * FROM post WHERE id = ?',(post_id,)).fetchone()
    print(post, current_user.id, post_id)
    if post:
